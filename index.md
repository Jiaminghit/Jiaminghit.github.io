...
- **项目背景：**  参与 **SACP智驾数据云平台** 多传感器预标注模块建设，面向**相机与激光雷达**采集数据，构建“云端读取-检测与跟踪—后处理—质检—云端回写”的自动化数据生产链路，为人工精修及下游感知模型训练提供动态、静态标注数据。
- **工作内容：**
  1.**动态标注2D质检功能：** 面向人工质检前的异常预筛，将相机 2D 检测接入 LiDAR 3D 动态标注链路，完成 3D 框多相机投影、Ego ROI 过滤及 2D–3D 实例关联；基于归一化重叠率与匈牙利算法实现全局匹配，并构建疑似误检、遮挡、视野外三类异常分流机制，设计3D准确率和2D召回率任务级质量指标。在8个测试生产数据**CLIPs（3200帧）** 中完成 **6.46 万组实例匹配**，主要动态类别（Car、Truck、Bicycle、Pedestrian）Precision、Recall代理指标分别达到**75.0%和84.5%**，自动标记 **1.21 万个疑似误检** 并补充 **1.18 万个疑似漏检** 线索，辅助人工聚焦异常结果。
  
  2.**标注后处理链路：** 封装基于 CTRV-EKF 与 RTS 平滑的动态轨迹插值、静态目标世界坐标鲁棒聚合算法，并接入动态标注链路，在测试用例中完成 **18.92万/18.95万个候选Feature** 转换，处理覆盖率达 **99.86%**；开发静态总标注到逐帧 Ego 标注切帧功能，结合 6DoF 坐标变换、五路点云融合及 KD-tree/OBB 筛选生成逐帧真值标注静态目标，将静态目标人工标注工作量压缩至原有的**1/44**。
  
  3.**云端生产接入：** 完成后处理与质检模块的可插拔集成，适配 S3 结果下载上传、Meta 写回、Kafka 状态上报以及 Docker/Argo 部署，补充异常降级、配置校验和自动化测试，开发功能已接入 SACP 云端生产流程进行约5.9w CLIPs的数据生产。
